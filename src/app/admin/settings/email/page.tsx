...
export default function EmailSettingsPage() {
  ...
  const fetchSettings = async (reveal = false) => {
    try {
      const url = reveal ? '/api/admin/settings/email?reveal=true' : '/api/admin/settings/email';
  ...
  const handleToggleApiKey = async () => {
    if (!showApiKey && settings.apiKey === '••••••••') {
      // Need to fetch the actual key
      setRevealingKey(true);
      try {
        const response = await fetch('/api/admin/settings/email?reveal=true');
        if (response.ok) {
          const data = await response.json();
